...
//   L3.10 — Email transport gates (added with 25-email):
//          (structural) src/lib/email.ts exports sendEmail / getEmailTransport
//              / NoopTransport / EmailTransport / EmailMessage;
//              src/lib/email-transport-ahasend.ts has the AhasendTransport
//              class + hardcoded api.ahasend.com endpoint;
//              src/lib/email-templates.tsx ships renderEmail / htmlToText /
//              escapeHtml AND retains the "DO NOT use Tailwind" banner
//              comment (Outlook/Gmail strip stylesheets — inline-style only).
//          (runtime)  Writes a throwaway TanStack server route into the live
//              dev tree that calls sendEmail() and returns the result, then
//              GETs it. With AHASEND_SECRET_KEY absent from .dev.vars the
//              transport selection drops to NoopTransport, so the returned
//              id starts with "noop-". The throwaway route is removed in a
//              finally block so the project tree stays clean.
//
//          Real-Ahasend sends are exercised manually (we don't want the
//          harness to spam an inbox on every run — that's a separate verify
//          step, see HANDOFF / install-step header).
//
...
try {
  ...
  // to pick up this route, so we plant it BEFORE the build. The matching
  // assertion + cleanup live at L3.10 (after dev server is up). If a test fails
  // before then, the cleanup runs anyway in the outer finally().
  const harnessEmailRoutePath = path.join(target, 'src', 'routes', 'api', 'harness-email-test.ts');
  const harnessEmailRouteSrc = `// AUTO-GENERATED by scripts/test.mjs L3.10 preflight — removed with tmpRoot.
import { createFileRoute } from '@tanstack/react-router';
import { sendEmail } from '../../lib/email';

export const Route = createFileRoute('/api/harness-email-test')({
  server: {
    handlers: {
      GET: async () => {
        const r = await sendEmail({
          to: 'l310-harness@example.local',
          subject: 'L3.10 noop gate',
          html: '<p>noop probe</p>',
          text: 'noop probe',
        });
...
`;
  await fs.writeFile(harnessEmailRoutePath, harnessEmailRouteSrc);
  ...
  // ─── L3.10 — email transport (no-op path) ──────────────────────────────
  // The 25-email step ships src/lib/email.ts with NoopTransport as the
  // fallback when AHASEND_SECRET_KEY isn't on the worker env. The dev
  // server's `.dev.vars` doesn't set Ahasend creds, so getEmailTransport()
  // is expected to return NoopTransport and sendEmail() should resolve with
  // an id starting with "noop-".
  //
  // We don't ship a permanent test endpoint — temporarily drop a tiny
  // TanStack server route into the live dev tree, wait for vite to pick it
  // up, fetch it, then remove. Vite's HMR re-runs the router plugin on file
  // adds inside src/routes/, so the route becomes serveable without
  // restarting the dev server.
  //
  // The real-Ahasend send path is exercised manually (we don't want to spam
  // an inbox on every harness run). Source-level structural checks below
  // gate that the AhasendTransport class + sendEmail / getEmailTransport
  // exports are present so a future regression that drops them is caught.
  console.log('[L3.10] Email transport (no-op gate + structural checks)...');

  // Structural gates on the rendered source — fast, run regardless of dev
  // server state.
  const emailFile = path.join(target, 'src', 'lib', 'email.ts');
  const emailSrc = await fs.readFile(emailFile, 'utf-8');
  for (const exportName of ['sendEmail', 'getEmailTransport', 'NoopTransport', 'EmailTransport', 'EmailMessage']) {
    if (!emailSrc.includes(exportName)) {
      throw new Error(`L3.10 src/lib/email.ts missing expected export "${exportName}"`);
    }
  }
  const transportFile = path.join(target, 'src', 'lib', 'email-transport-ahasend.ts');
  const transportSrc = await fs.readFile(transportFile, 'utf-8');
  if (!transportSrc.includes('AhasendTransport') || !transportSrc.includes('api.ahasend.com/v2/accounts/')) {
    throw new Error('L3.10 src/lib/email-transport-ahasend.ts missing AhasendTransport class or Ahasend endpoint URL');
  }
  const templatesFile = path.join(target, 'src', 'lib', 'email-templates.tsx');
  const templatesSrc = await fs.readFile(templatesFile, 'utf-8');
  if (!templatesSrc.includes('renderEmail') || !templatesSrc.includes('htmlToText') || !templatesSrc.includes('escapeHtml')) {
    throw new Error('L3.10 src/lib/email-templates.tsx missing renderEmail / htmlToText / escapeHtml exports');
  }
  // Banner contract — protect future contributors from a Tailwind refactor.
  if (!templatesSrc.includes('DO NOT use Tailwind classes in email templates')) {
    throw new Error('L3.10 email-templates.tsx is missing the "DO NOT use Tailwind" banner comment');
  }
  console.log('       (structural) OK — email exports + AhasendTransport + template banner present');

  // Live no-op gate. The throwaway route file was planted before L2 so the
  // router plugin's routeTree.gen.ts (built in L2 and re-built on dev boot)
  // includes it. GET it; assert the response is the noop transport's
  // synthetic id shape. Cleanup happens in the outer finally() (see
  // harnessEmailRoutePath at top of try block).
  const probeUrl = `${origin}/api/harness-email-test`;
  let probeRes = null;
  let lastStatus = null;
  ...
  while (Date.now() < probeDeadline) {
    try {
      ...
      // ignore transient network errors during dev-server warmup
    }
    await new Promise((r) => setTimeout(r, 250));
  }
  if (!probeRes || !probeRes.ok) {
    throw new Error(`L3.10 throwaway route never returned 200 (last status: ${lastStatus})`);
  }
  const probeBody = await probeRes.json();
  if (typeof probeBody?.id !== 'string' || !probeBody.id.startsWith('noop-')) {
